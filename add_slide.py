...
def extract_mosaic(index, slide):
    i, j = index
    patch = slide.read_region((i * 224, j * 224), 0,
                              (224, 224)).convert('RGB')
    patch = cv2.cvtColor(np.array(patch), cv2.COLOR_RGB2BGR)
    mosaic = utils.extract_patches(patch)
    return (mosaic, index)

def add_slide(path, model, r):
    ...
    with Parallel(n_jobs=num_cores, prefer="threads") as parallel:
        ...
        nbr_iter = len(patch_list) // max_tensor_size
        s = [slide] * max_tensor_size

        for i in range(nbr_iter):
            result = parallel(delayed(read_rgb_regions)(index, j)
                              for index, j in zip(patch_list[max_tensor_size * i : max_tensor_size * (i+1)], s))
            for j in range(max_tensor_size):
                tensor_cpu[j] = transforms.ToTensor()(cv2.cvtColor(np.array(result[j]),
                                                               cv2.COLOR_RGB2BGR))
            tensor_gpu = transform(tensor_cpu.to(device='cuda:0'))
            out = model(tensor_gpu)
            tensor_out[max_tensor_size * i : max_tensor_size * (i+1)] = transforms.ToTensor()(out)

        rest = len(patch_list) % max_tensor_size

        if rest != 0:
            s = [slide] * rest
            result = parallel(delayed(read_rgb_regions)(index, j) \
                              for index, j in zip(patch_list[max_tensor_size * nbr_iter:], s))
            for j in range(rest):
                tensor_cpu[j] = transforms.ToTensor()(cv2.cvtColor(np.array(result[j]),
                                                               cv2.COLOR_RGB2BGR))
            tensor_gpu = transform(tensor_cpu.to(device='cuda:0'))
            out = model(tensor_gpu)
            tensor_out[max_tensor_size * nbr_iter:] = transforms.ToTensor()(out[:rest])
        ...
        nbr_iter = len(final_patch_list) // max_tensor_size
        s = [slide] * max_tensor_size

        counter = 0

        for i in range(nbr_iter):
            result = parallel(delayed(extract_mosaic)(index, j)
                              for index, j in zip(final_patch_list[max_tensor_size * i : max_tensor_size * (i+1)], s))
            ...
            for res in result:
                ...
                for j in range(len(mosaic)):
                    ...
                    if counter == max_tensor_size:
                        tensor_gpu = transform(tensor_cpu.to(device='cuda:0'))
                        out = model(tensor_gpu)
                        for l in range(max_tensor_size):
                            abscissa, ordinate = coordinates_list[l]
                            r.lpush(np.array2string(out[l]),
                                    json.dumps({"name": path, "x": abscissa,
                                                "y": ordinate}))
